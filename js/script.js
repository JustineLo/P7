...
function getFilterAllDOMElements(filterCategory) {
  const filterContainer = document.getElementById(
    `${filterCategory}-filter-container`
  );
...
function displayFilterList(filterCategory, inputString) {
  const filterListDOM = document.getElementById(
    `${filterCategory}-filter-list`
  );
  let filterListItems = [];
  ...
  if (inputString) {
    filterListItems = filterListItems.filter((item) =>
      item.toLocaleLowerCase().includes(inputString.toLocaleLowerCase())
    );
  }
  ...
  filterListItems.map((item) => {
    ...
    button.addEventListener("click", () => {
      ...
      displayedRecipes = getFilteredRecipes(
        displayedRecipes,
        item,
        filterCategory
      );
...
function getSearchedRecipes(inputString, displayedRecipes) {
  const results = displayedRecipes.filter((recipe) => {
    ...
    const checkRecipeName = recipe.name
      .toLowerCase()
      .includes(inputStringLowerCase);
    const checkRecipeIngredients = recipe.ingredients.some((ingredient) =>
      ingredient.ingredient.toLowerCase().includes(inputStringLowerCase)
    );
    const checkRecipeDescription = recipe.description
      .toLowerCase()
      .includes(inputStringLowerCase);
